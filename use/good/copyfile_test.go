package main

import (
	"fmt"

	"github.com/michaelmacinnis/escape/use/errors"
)

/*
This function is meant to emulate the CopyFile function from:

https://go.googlesource.com/proposal/+/master/design/go2draft-error-handling-overview.md

	func CopyFile(src, dst string) error {
		handle err {
			return fmt.Errorf("copy %s %s: %v", src, dst, err)
		}

		r := check os.Open(src)
		defer r.Close()

		w := check os.Create(dst)
		handle err {
			w.Close()
			os.Remove(dst) // (only if a check fails)
		}

		check io.Copy(w, r)
		check w.Close()
		return nil
	}
*/
func mockCopyFile(data map[string]error) (err error) {
	var escape_hatch_1 func(error)
	{
		p := &err

		panicking := false

		escape_hatch_1 = func(v error) { // generated by escape: copyfile_test.ego:34:31
			*p = v

			if !panicking {
				panicking = true
				panic(&panicking)
			}
		}

		defer func() {
			if panicking {
				panicking = false

				r := recover()
				if r != &panicking {
					// It would be better if we could unrecover.
					// Or if panicking with the return value from recover, unrecovered.
					// Or if there was a mechanism specifically for this that did not
					// use panic/recover but was weaker than panic in the same way that
					// panic is weaker than runtime.Goexit.
					panic(r)
				}
			}
		}()
	}
	check := errors.Check(escape_hatch_1)

	defer errors.Handle(&err, func() {
		err = fmt.Errorf("copy(src, dst): %w", err)
	})

	err = mock(data, "open(src)")
	check(err)

	defer mock(data, "close(src)")

	err = mock(data, "open(dst)")
	check(err)

	defer errors.Handle(&err, func() {
		mock(data, "close(dst)")
		mock(data, "remove(dst)")
	})

	err = mock(data, "copy(dst, src)")
	check(err)

	return mock(data, "close(dst)")
}

func mock(data map[string]error, name string) error {
	fmt.Printf("%s\n", name)
	return data[name]
}

func wrapMockCopyFile(data map[string]error) {
	err := mockCopyFile(data)
	if err != nil {
		fmt.Printf("%s\n", err.Error())
	}
}

func ExampleCopyFile_close_dst_err() {
	wrapMockCopyFile(map[string]error{
		"close(dst)": errors.New("problem closing dst"),
	})
	// Output:
	// open(src)
	// open(dst)
	// copy(dst, src)
	// close(dst)
	// close(dst)
	// remove(dst)
	// close(src)
	// copy(src, dst): problem closing dst
}

func ExampleCopyFile_copy_err() {
	wrapMockCopyFile(map[string]error{
		"copy(dst, src)": errors.New("problem copying"),
	})
	// Output:
	// open(src)
	// open(dst)
	// copy(dst, src)
	// close(dst)
	// remove(dst)
	// close(src)
	// copy(src, dst): problem copying
}

func ExampleCopyFile_no_dst() {
	wrapMockCopyFile(map[string]error{
		"open(dst)": errors.New("dst not found"),
	})
	// Output:
	// open(src)
	// open(dst)
	// close(src)
	// copy(src, dst): dst not found
}

func ExampleCopyFile_no_error() {
	wrapMockCopyFile(map[string]error{})
	// Output:
	// open(src)
	// open(dst)
	// copy(dst, src)
	// close(dst)
	// close(src)
}

func ExampleCopyFile_no_src() {
	wrapMockCopyFile(map[string]error{
		"open(src)": errors.New("src not found"),
	})
	// Output:
	// open(src)
	// copy(src, dst): src not found
}
